...
export default function App(){
    ...
    async function makeProblemSet(){
        ...
        const problemSetSize = selectedProblems.reduce((sum, { count }) => sum + Number(count), 0);
        ...
        const userExcludeQuery = excludedUsers.map(user => "-s%40" + user).join("%20");
        const tagSelectQuery = selectedTags.map(tag => "%23" + tag.key).join("%7C");
        try{
            const responses = await Promise.all(selectedProblems.map(({ tierIndex }) => {
                const optionIndex = Math.floor(tierIndex / 6);
                const subIndex = tierIndex % 6;
                const problemQuery = `*${"bsgpdr"[optionIndex]}${subIndex ? "54321"[subIndex - 1] : ""}`;
                const query = `${problemQuery}%20${userExcludeQuery}(${tagSelectQuery})`;
                return axios.get(`${SOLVEDAC_API_ENDPOINT}?query=${query}&sort=random`);
            }));
            const [newProblemSet, insufficientProblems] = tryMakingProblemset(selectedProblems, responses);
            ...
            if(error.response && error.response.status === 400)
                ...
                setErrorMessageList([errorMessages.ERR_FAILED_TO_LOAD]);
        }
        finally{
...
function extractSelectedProblems(problemOption){
    ...
    for(let optionIndex = 0; optionIndex < 6; optionIndex++){
        ...
        for(let subIndex = 0; subIndex < 6; subIndex++){
            ...
            selectedProblems.push({ tierIndex: optionIndex * 6 + subIndex, count: count[subIndex] });
        }
    }
